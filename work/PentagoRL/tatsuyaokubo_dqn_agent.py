...
class TatsuyaokuboDqnAgent():
    ...
    def build_network(self):
        model = Sequential()
        model.add(Dense(64, activation='relu', input_dim= FRAME_WIDTH * FRAME_HEIGHT))
...
#        model.add(Dense(self.num_actions))
        ...
        s = tf.placeholder(tf.float32, [None, FRAME_WIDTH * FRAME_HEIGHT])
        q_values = model(s)
...
#        return np.stack(state, axis=0)
    ...
    def run(self, state, action, reward, terminal, observation):
        next_state = observation #np.append(state[1:], observation, axis=0)
...
#        return action
    ...
    def act(self, state, verbose=False):
        return self.get_action(state.reshape((FRAME_WIDTH * FRAME_HEIGHT)))
        
    def learn(self, state, action, state_next, reward, done, info, verbose=False):
        _ = self.run(state.reshape((FRAME_WIDTH * FRAME_HEIGHT)), action, reward, done, state_next.reshape((FRAME_WIDTH * FRAME_HEIGHT)))
        return
...
#    agent = TatsuyaokuboDqnAgent(num_actions=env.action_space.n)
#
#    if TRAIN:  # Train mode
